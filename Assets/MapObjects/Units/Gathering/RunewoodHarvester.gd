...
export var WalkSpeed : int = 15
#export var ResourceGenerationAmount := 0
onready var navagent : NavigationAgent = $NavigationAgent
var runOnce := true
#export(String, "Voidstone", "Runewood", "Ethertite", "AtekGas") var ResourceNameToGet
...
func _process(delta):
	match CurrentTask:
		...
		Task.Searching:
			var resources = get_tree().get_nodes_in_group("Runetree")
			if resources.size() > 0:
...
#						"tree":
					GameManager.Runewood += HeldResourceAmount
#						"stone":
